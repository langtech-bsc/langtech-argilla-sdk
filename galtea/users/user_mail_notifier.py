...
class UserEmailNotifier:
    ...
    def send_user_credentials_email(
        ...
        argilla_url: str = os.getenv("ARGILLA_API_URL"),
    ):
    ...
    def send_user_credentials(self, credentials: dict) -> bool:
        print(
            f"Sending credentials to email {credentials['email']}, username: {credentials['username']}"
        )
        return self.send_user_credentials_email(
